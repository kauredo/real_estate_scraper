  ...
  module V1
    module SuperAdmin
      class TenantsController < Api::V1::BaseController
        ...
        before_action :set_tenant, only: [:show, :update, :destroy, :toggle_active, :rotate_api_key]

        def index
          tenants = ::Tenant.order(:name)
          render json: {
            tenants: tenants.as_json(
              only: [:id, :name, :slug, :domain, :active, :contact_email, :created_at],
        ...
        def show
          render json: {
            tenant: @tenant.as_json(
              only: [:id, :name, :slug, :domain, :active, :contact_email, :created_at, :updated_at],
              methods: [:enabled_features]
            )
          }
        end
        ...
        def create
          ...
          if tenant.save
            render json: {
              tenant: tenant.as_json(
                only: [:id, :name, :slug, :domain, :active, :contact_email, :api_key, :created_at],
                methods: [:enabled_features]
              ),
        ...
        def tenant_params
          params.require(:tenant).permit(:name, :slug, :domain, :contact_email, :active)
        end
