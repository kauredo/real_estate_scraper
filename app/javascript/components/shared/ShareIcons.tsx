...
export default function ShareIcons(props: Props) {
  ...
  return (
    <div className="mx-auto mb-4 font-bold flex items-center gap-2 h-5 w-fit text-xl">
      ...
      <div className="hover:text-beige" title={i18n.t("share.whatsapp")}>
        <WhatsappShareButton url={shareUrl} title={title} separator=":: ">
          <FontAwesomeIcon icon={faWhatsapp as IconProp} />
        </WhatsappShareButton>
      </div>
      <div className="hover:text-beige" title={i18n.t("share.messenger")}>
        <FacebookMessengerShareButton url={shareUrl} appId="521270401588372">
          <FontAwesomeIcon icon={faFacebookMessenger as IconProp} />
        </FacebookMessengerShareButton>
      </div>
      <div className="hover:text-beige" title={i18n.t("share.telegram")}>
        <TelegramShareButton url={shareUrl} title={title}>
          <FontAwesomeIcon icon={faTelegram as IconProp} />
        </TelegramShareButton>
      </div>
      <div className="hover:text-beige" title={i18n.t("share.facebook")}>
        <FacebookShareButton url={shareUrl}>
          <FontAwesomeIcon icon={faFacebook as IconProp} />
        </FacebookShareButton>
      </div>
      <div className="hover:text-beige" title={i18n.t("share.twitter")}>
        <TwitterShareButton url={shareUrl} title={title}>
          <FontAwesomeIcon icon={faTwitter as IconProp} />
        </TwitterShareButton>
      </div>
      <div className="hover:text-beige" title={i18n.t("share.email")}>
        <EmailShareButton url={shareUrl} subject={title}>
          <FontAwesomeIcon icon={faEnvelope as IconProp} />
        </EmailShareButton>
      </div>
      <div className="hover:text-beige">
