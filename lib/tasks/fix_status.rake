...
task fix_duplicates: :environment do
  ...
  grouped_listings = portuguese_listings.group('translations.title').count
  ...
  titles_and_counts.each do |title, _count|
    listings_for_title = portuguese_listings.joins(:translations).where(translations: { title: }).to_a
